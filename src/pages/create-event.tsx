...
export default function CreateEvent() {
  ...
  useEffect(() => {
    ...
    const userId = 1;
    fetchUserRSOs(userId);
    fetchEventCategories();
    fetchEventTypes();
  }, []);

  const fetchUserRSOs = async (userId: number) => {
    const response = await fetch(`/api/user_rso/${userId}`);
    if (response.ok) {
      const data = await response.json();
      setUserRSOs(data);
    } else {
      console.error('Failed to fetch user RSOs');
    }
  };  

  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!selectedEventCategoryId || !selectedEventTypeId || !startTime || !endTime || !address || !locationName) {
